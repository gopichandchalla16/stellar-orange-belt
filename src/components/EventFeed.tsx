...
interface Props { events: StellarEvent[]; }
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        {events.map((e, i) => (
          <div key={i} style={{ padding: '12px 14px', borderRadius: 12, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
            ...
            {e.type === 'contrib' && <p style={{ fontSize: 12, color: '#94A3B8' }}><b style={{ color: '#fff' }}>{e.amount} XLM</b> from {e.from} to Campaign #{e.campaignId}</p>}
            {e.type === 'created' && <p style={{ fontSize: 12, color: '#94A3B8' }}>&ldquo;{e.title}&rdquo;</p>}
          </div>
        ))}
      </div>
    </div>
