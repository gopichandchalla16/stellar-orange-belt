      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '24px 16px' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 40, padding: '20px 0' }}>
          <h1 style={{ fontSize: 'clamp(28px,5vw,52px)', fontWeight: 900, lineHeight: 1.1, marginBottom: 16 }}>
            <span style={{ background: 'linear-gradient(90deg,#F97316,#FB923C,#FCD34D)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Decentralized</span>{' '}Crowdfunding
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12, marginBottom: 32 }}>
          {([['🏆', 'Total Campaigns', campaigns.length], ['🔥', 'Active', campaigns.filter(c => daysLeft(c.deadline) > 0).length], ['💰', 'Total Raised XLM', campaigns.reduce((a, c) => a + c.raised, 0).toLocaleString()]] as [string, string, string | number][]).map(([icon, label, val]) => (
