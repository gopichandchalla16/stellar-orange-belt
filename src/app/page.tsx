...
export default function Home() {
  ...
  const [events,      setEvents]      = useState<Array<{ type: string; campaignId?: number; amount?: number; from?: string; title?: string; ts: number }>>([]);
  ...
  const handleContribute = async (c: Campaign) => {
    ...
    try {
      ...
      setCampaigns(prev =>
        prev.map(p => p.id === c.id ? { ...p, raised: Math.min(p.raised + 100, p.goal) } : p)
      );
      setTxHash(hash);
      setTxStatus('success');
  ...
  return (
    <main style={{ minHeight: '100vh', background: 'linear-gradient(160deg,#080C18 0%,#0D1426 50%,#060A14 100%)' }}>
      ...
      <header style={{ borderBottom: '1px solid rgba(249,115,22,0.15)', padding: '0 20px', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between', maxWidth: 1100, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          {wallet && <span style={{ fontSize: 12, color: '#94A3B8' }}>{balance} XLM</span>}
          {wallet ? (
            ...
            <button onClick={handleConnect} disabled={loading} className="btn-orange" style={{ width: 'auto', padding: '9px 18px', fontSize: 13 }}>
              {loading ? 'Connecting...' : hasFreighter ? 'Connect Wallet' : 'Install Freighter'}
            </button>
          )}
        </div>
      </header>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '24px 16px' }}>
        ...
        {/* Error Banner */}
        {error && (
          <div style={{ marginBottom: 16, padding: '12px 16px', borderRadius: 12, background: 'rgba(248,113,113,0.08)', border: '1px solid rgba(248,113,113,0.25)', color: '#FCA5A5', fontSize: 13, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span>&#9888; {error}</span>
        ...
        {/* TX Status */}
        {txStatus !== 'idle' && (
          <div style={{ marginBottom: 16, padding: '14px 18px', borderRadius: 14, border: `1px solid ${txStatus === 'pending' ? 'rgba(249,115,22,0.3)' : txStatus === 'success' ? 'rgba(52,211,153,0.3)' : 'rgba(248,113,113,0.3)'}`, background: txStatus === 'pending' ? 'rgba(249,115,22,0.07)' : txStatus === 'success' ? 'rgba(52,211,153,0.07)' : 'rgba(248,113,113,0.07)', display: 'flex', alignItems: 'center', gap: 12 }}>
            <span style={{ fontSize: 18 }}>{txStatus === 'pending' ? '\u23f3' : txStatus === 'success' ? '\u2705' : '\u274c'}</span>
            <div style={{ flex: 1 }}>
              ...
              {txStatus === 'success' && txHash && (
                <a href={`https://stellar.expert/explorer/testnet/tx/${txHash}`} target="_blank" rel="noreferrer" style={{ fontSize: 11, color: '#F97316', fontFamily: 'monospace' }}>
                  {txHash.slice(0, 40)}...
                </a>
              )}
            </div>
            {txStatus !== 'pending' && (
              <button onClick={() => { setTxStatus('idle'); setTxHash(''); }} style={{ background: 'none', border: 'none', color: '#64748B', cursor: 'pointer', fontSize: 16 }}>&#10005;</button>
            )}
          </div>
        )}
        ...
        <div style={{ textAlign: 'center', marginBottom: 40, padding: '20px 0' }}>
          <h1 style={{ fontSize: 'clamp(28px,5vw,52px)', fontWeight: 900, lineHeight: 1.1, marginBottom: 16 }}>
            ...
            {' '}Crowdfunding
            <br />on Stellar Soroban
          </h1>
          <p style={{ color: '#94A3B8', fontSize: 'clamp(14px,2vw,17px)', maxWidth: 520, margin: '0 auto 24px' }}>
            Create campaigns, fund projects with XLM, governed by on-chain Soroban smart contracts.
          </p>
          {wallet && (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12, marginBottom: 32 }}>
          {([
            ...
            ['\ud83d\udd25', 'Active', campaigns.filter(c => daysLeft(c.deadline) > 0).length],
            ['\ud83d\udcb0', 'Total Raised XLM', campaigns.reduce((a, c) => a + c.raised, 0).toLocaleString()],
          ] as [string, string, string | number][]).map(([icon, label, val]) => (
                ...
                <div key={c.id} className="card animate-fade-in">
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                    ...
                    <span style={{
                      ...
                      background: daysLeft(c.deadline) > 0 ? 'rgba(52,211,153,0.1)' : 'rgba(248,113,113,0.1)',
                      color:      daysLeft(c.deadline) > 0 ? '#34D399' : '#F87171',
                      border:     `1px solid ${daysLeft(c.deadline) > 0 ? 'rgba(52,211,153,0.25)' : 'rgba(248,113,113,0.25)'}`,
                    }}>
                  ...
                  <button
                    ...
                    disabled={!wallet || txStatus === 'pending' || pct(c) >= 100}
